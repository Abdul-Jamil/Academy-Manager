...
@Service
public class AddScheduleEntryUseCase {
    ...
    public AddScheduleEntryUseCase(
    ...
    @Transactional
    public ScheduleEntry execute(UUID scheduleId, DayOfWeek dayOfWeek, int periodNumber, LocalTime startTime, LocalTime endTime, UUID courseId, String teacherId) {
        ...
        if (scheduleEntryRepository.existsEntryForScheduleDayAndPeriod(
            ...
            throw new IllegalStateException(
                    "This period is already assigned in the schedule"
            );
        }
        ...
        for (int i = 0; i < MAX_RETRIES; i++) {
            ScheduleEntryId entryId = ScheduleEntryId.newId();
        ...
        throw new IllegalStateException(
                "Could not create schedule entry after several retries"
        );
    }
}
