...
public class ActivateScheduleUseCase {
    ...
    public ActivateScheduleUseCase(
    ...
    public ClassSchedule execute(String userId, UUID scheduleId) {
        ...
        if (schedule.isActive()) {
            throw new IllegalStateException("Schedule is already active");
        }

        /*
         * Repository should prevent two active schedules for the
         * same class from having overlapping effective periods.
         */
        if (classScheduleRepository.existsActiveOverlappingSchedule(
                schedule.getClassId(),
                schedule.getEffectiveFrom(),
                schedule.getEffectiveTo(),
                schedule.getId()
        )) {
            throw new IllegalStateException("Another active schedule overlaps this schedule's effective period");
        }
